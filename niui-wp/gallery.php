...
*/
			...
			$content .= '<li id="slide-' . $attachment->ID . '"><figure><picture>' . wp_get_attachment_image( $attachment->ID, ['thumbnail', 'large', 'original']) . '</picture><figcaption>' . wp_get_attachment_caption( $attachment->ID) . '</figcaption></figure></li>';
			/* Close gallery row. */
/*
			if ( $this->args['columns'] > 0 && ++$i % $this->args['columns'] == 0 )
				$output .= "";
...
<script> // If the featured image is also in a lightbox, open it on click
	...
	if (thumbnail) {
		...
		return "\n\t\t\t" . sprintf( '<div class="n-carousel n-carousel--lightbox ' . ($this->args['columns'] == '9' ? '' : 'n-carousel--inline') . '">' . $content . '<div %s>', $gallery_attr ) . str_replace( 'http://', '//', $output ) . "\n\t\t\t</div><div class='n-carousel__full-screen'><button><span>Toggle full screen</span></button>
