function addClass(b,a){if(b.classList){b.classList.add(a)}else{b.className+=" "+a}}function removeClass(b,a){if(b.classList){b.classList.remove(a)}else{b.className=b.className.replace(new RegExp("(^|\\b)"+a.split(" ").join("|")+"(\\b|$)","gi")," ")}}function hasClass(b,a){if(b.classList){return b.classList.contains(a)}else{return new RegExp("(^| )"+a+"( |$)","gi").test(b.className)}}function toggleClass(b,a){if(hasClass(b,a)){removeClass(b,a)}else{addClass(b,a)}}function eventElement(a){a=a||window.event;
return a.target||a.srcElement}var parseHTML=function(a){tmp=document.implementation.createHTMLDocument("Parsed");tmp.body.innerHTML=a;return tmp.body};function forEach(a,c){elements=(typeof a=="string")?document.querySelectorAll(a):a;for(var b=0;b<elements.length;b++){c(elements[b],b)}}function addEventHandler(c,a,b){if(c.addEventListener){c.addEventListener(a,b,false)}else{if(c.attachEvent){c.attachEvent("on"+a,b)}}}function stopEvent(a){if(!a){var a=window.event;if(typeof a=="undefined"){return
}}a.cancelBubble=true;a.returnValue=false;if(a.stopPropagation){a.stopPropagation()}if(a.preventDefault){a.preventDefault()}return false}function thisIndex(c){if(!c){return}var a=c.parentNode.childNodes,d;var b=count=0;while((d=a.item(b++))&&d!=c){if(d.nodeType==1){count++}}return(count)}function parentByClass(b,a){while(b.parentNode&&!hasClass(b,a)){b=b.parentNode}if(hasClass(b,a)){return b}else{return null}}forEach("table",function(b,a){b.insertAdjacentHTML("beforebegin",'<div class="table">'+b.outerHTML+"</div>");
b.outerHTML=""});function updateURLParameter(b,d,g){var j="";var f=b.split("?");var c=f[0];var e=f[1];var h="";if(e){f=e.split("&");for(i=0;i<f.length;i++){if(f[i].split("=")[0]!=d){j+=h+f[i];h="&"}}}var a=h+""+d+"="+g;return c+"?"+j+a}function getURLParameters(){var a={},b=/[?&]([^?&]+)=([^?&]+)/g;location.href.replace(b,function(e,d,c){a[d]=c});return a}var parameters_list=new Array("parameter1","parameter2");function relayParameters(){parameters=getURLParameters();forEach("a[href]",function(c,b){for(var a in parameters){if(parameters_list.indexOf(a)==-1){continue
...
}function modalWindow(d){el=eventElement(d);if(parentByClass(el,"modal")&&!(new XMLHttpRequest().upload)){el=parentByClass(el,"modal");window.open(el.href,"_blank");return false}document.body.onkeyup=function(f){f=f||window.event;if(f.keyCode==27){removeBlackbox()}};document.body.insertAdjacentHTML("afterbegin",'<div id="blackbox"> </div>');addClass(document.querySelector("html"),"nooverflow");if(typeof d=="string"){document.getElementById("blackbox").innerHTML='<div class="close"> ← '+document.title+"</div>"+d+'<div id="blackbox-bg"></div>';
document.getElementById("blackbox-bg").onclick=document.querySelector("#blackbox .close").onclick=removeBlackbox;return false}if(parentByClass(el,"modal")){el=parentByClass(el,"modal");request=new XMLHttpRequest();request.open("GET",el.href.split("#")[0],true);request.onload=function(){if(request.status>=200&&request.status<400){container=(typeof el.href.split("#")[1]!="undefined")?("#"+el.href.split("#")[1]):0;blackbox=document.getElementById("blackbox");blackbox.insertAdjacentHTML("afterbegin",'<div class="close"> ← '+document.title+"</div>");
...
a+=c.offsetTop}while(c=c.offsetParent)}return{x:b,y:a}};function animateAnchors(b){b=b||window.event;if(typeof b=="undefined"){return}var a=b.target||b.srcElement;hash=document.getElementById(a.href.split("#")[1]);document.querySelector("#nav-trigger").checked=false;removeClass(document.querySelector(".nav-main > div"),"open");removeClass(document.getElementById("content"),"semi-transparent");removeClass(document.getElementById("footer"),"semi-transparent");removeClass(document.querySelector("#head > .row"),"semi-transparent");
...
stopEvent(a);el=el.parentNode;toggleClass(el,"open");el.querySelector("div").style.maxHeight=((el.querySelector("div").style.maxHeight=="")?(el.querySelector("div").scrollHeight+"px"):"");if(hasClass(el.parentNode.parentNode,"accordion")){el.parentNode.style.maxHeight=el.querySelector("div").scrollHeight+el.parentNode.scrollHeight+"px"}return false}forEach(".accordion > label",function(b,a){b.onclick=toggleAccordion;b=b.parentNode;if(b.querySelector("input.trigger")){b.querySelector("input.trigger").outerHTML=""
}b.querySelector("div").onclick=function(c){c=c||window.event;c.cancelBubble=true}});if("ontouchstart" in window){document.body.insertAdjacentHTML("beforeend","<style> a[href]:hover { color: inherit; } .tool:hover .tip { display: none; } </style>")}document.getElementById("nav-trigger").onchange=function(a){toggleClass(document.getElementById("content"),"semi-transparent");toggleClass(document.getElementById("footer"),"semi-transparent");toggleClass(document.querySelector("#head > .row"),"semi-transparent")
};addEventHandler(window,"load",function(){var a=22;forEach("#content img",function(b){extra_padding=((Math.round(b.height/a)+1)*a-b.height);if(extra_padding>=a){extra_padding-=a}b.style.paddingBottom=extra_padding+"px"})});var new_event_support=1;try{var test_event=new Event("test")}catch(err){new_event_support=0}function sliderElement(a){el=eventElement(a);if(hasClass(el,"slider-container")){el=el.querySelector(".slider")}else{while(!hasClass(el,"slider")){el=el.parentNode}}return el}var lastAnimation=0;
swipeEvents=function(d){var b,a;d.addEventListener("touchstart",c);function c(f){var g=f.touches;if(g&&g.length){b=g[0].pageX;a=g[0].pageY;d.addEventListener("touchmove",e)}}function e(h){var j=h.touches;if(j&&j.length){var g=b-j[0].pageX;var f=a-j[0].pageY;if((Math.abs(g)<Math.abs(f))&&!document.querySelector(".slider.lightbox")){return}h.preventDefault();if(g>=50){if(new_event_support){var h=new Event("swipeLeft");d.dispatchEvent(h)}else{initScroll(h,-1*g)}}if(g<=-50){if(new_event_support){var h=new Event("swipeRight");
d.dispatchEvent(h)}else{initScroll(h,-1*g)}}if(Math.abs(g)>=50||Math.abs(f)>=50){d.removeEventListener("touchmove",e)}}}};initScroll=function(d,e){var b=e,a=new Date().getTime();if((a-lastAnimation)<800){d.preventDefault();return}var c=sliderElement(d);slide(c,(b<0)?"right":"left");lastAnimation=a};mouseWheelHandler=function(a){var b=(a.deltaX*-10)||a.wheelDeltaX||-a.detail;if(Math.abs(b)<50){return}a.preventDefault();initScroll(a,b)};function mouseEvents(b,a){if(!("onwheel" in window)){return}if(a=="off"){b.parentNode.removeEventListener("wheel",mouseWheelHandler)
...
if(a==0&&g=="left"){a=c.children.length}if(a==(c.children.length-1)&&g=="right"){a=-1}pos=((g=="left"?--a:++a)*100)+"%"}removeClass(c.parentNode.querySelector(".slider-nav .active"),"active");addClass(c.parentNode.querySelector(".slider-nav").children[a],"active");mouseEvents(el.parentNode,"off");var f=400;if(("ontransitionend" in window)||(typeof document.body.style.MozTransition=="string")){c.style.cssText="overflow-y: visible; -webkit-transform: translateX(-"+pos+"); -moz-transform: translateX(-"+pos+"); -ms-transform: translateX(-"+pos+"); transform: translateX(-"+pos+"); -webkit-transition: -webkit-transform "+f+"ms ease; -moz-transition: -moz-transform "+f+"ms ease; -ms-transition: -ms-transform "+f+"ms ease;";
c.addEventListener("transitionend",function(h){mouseEvents(el.parentNode);document.onkeyup=sliderKeyboard},false)}else{c.style.cssText="overflow-y: visible; left: -"+pos+"; -webkit-transition: left "+f+"ms ease; transition: left "+f+"ms ease;";var b=setTimeout(function(){mouseEvents(el.parentNode);document.onkeyup=sliderKeyboard},f)}}function sliderKeyboard(b){b=b||window.event;if(typeof b=="undefined"){return}el=b.target||b.srcElement;if(document.querySelector(".slider")==null){return}var a=el.tagName.toLowerCase();
el=document.querySelector(".slider.lightbox")||document.querySelector(".slider");switch(event.which){case 37:if(a!="input"&&a!="textarea"){slide(el,"left")}break;case 39:if(a!="input"&&a!="textarea"){slide(el,"right")}break;default:return}document.onkeyup=function(){}}function makeSlider(c,d){if(c.children.length<2){return c}addClass(c,"slider");c.insertAdjacentHTML("beforebegin",'<div class="slider-container"></div>');container=c.previousSibling;container.insertAdjacentHTML("afterbegin",'<a class="slider-arrow left">←</a>'+c.outerHTML+'<a class="slider-arrow right">→</a><div class="slider-nav"></div>');
