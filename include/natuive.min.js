...
swipeEvents=function(d){var b,a;d.addEventListener("touchstart",c);function c(g){var f=g.touches;if(f&&f.length){b=f[0].pageX;a=f[0].pageY;d.addEventListener("touchmove",e)}}function e(k){var j=k.touches;if(j&&j.length){var g=b-j[0].pageX;var f=a-j[0].pageY;if((Math.abs(g)<Math.abs(f))&&!document.querySelector(".slider.lightbox")){return}k.preventDefault();if(g>=50){if(new_event_support){var h=new Event("swipeLeft");d.dispatchEvent(h)}else{initScroll(k,-1*g)}}if(g<=-50){if(new_event_support){var h=new Event("swipeRight");
d.dispatchEvent(h)}else{initScroll(k,-1*g)}}if(Math.abs(g)>=50||Math.abs(f)>=50){d.removeEventListener("touchmove",e)}}}};initScroll=function(d,e){var b=e,a=new Date().getTime();if((a-lastAnimation)<800){d.preventDefault();return}var c=sliderElement(d);slide(c,(b<0)?"right":"left");lastAnimation=a};mouseWheelHandler=function(a){var b=(a.deltaX*-10)||a.wheelDeltaX||-a.detail;if(Math.abs(b)<50){return}a.preventDefault();initScroll(a,b)};function mouseEvents(b,a){if(!("onwheel" in window)){return}if(a=="off"){b.parentNode.removeEventListener("wheel",mouseWheelHandler)
