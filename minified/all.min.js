...
function slideEnd(){slider.onscroll=scrollSlider;clearTimeout(scrollTimer);removeClass(document.body,'disable-hover');moveIndex();original_scroll=slider.scrollLeft;}
...
addEventHandler(window,'load',function(){document.onkeyup=function(e){slider=document.querySelector('.slider');var event=e||window.event;if(event.keyCode==37){slide(e,'left');}
if(event.keyCode==39){slide(e,'right');}};forEachElement('.slider',function(el,i){if(!i){slider=el;}
el.insertAdjacentHTML('beforebegin','<div class="slider-container"></div>');container=el.previousSibling;container.insertAdjacentHTML('afterbegin','<a class="slider-arrow left">   </a>'+el.outerHTML+'<a class="slider-arrow right">   </a><div class="slider-nav"></div>');container.nextSibling.outerHTML='';el=container.querySelector('.slider');for(var i=0;i<el.children.length;i++){container.querySelector('.slider-nav').insertAdjacentHTML('beforeend',(!i?'<a class="active">':'<a>')+(i+1)+'</a>');container.querySelector('.slider-nav').lastChild.onclick=function(e){slide(e,'index');};}
